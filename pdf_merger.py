...
def get_input_pdfs(pdf_folder, pdf_files):
    if pdf_files is not None:
        return pdf_files
    elif pdf_folder is not None:
        all_files = os.listdir(pdf_folder)
        pdf_files = sorted(filter(lambda file: file.lower().endswith(".pdf"), all_files))
...
def parse_args():
    ...
    return get_input_pdfs(args.folder, args.input_pdfs), args.output_pdf
